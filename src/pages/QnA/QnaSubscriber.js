            ...
            setWaitList((prevList) => {
              switch (status) {
                ...
                case "ADD": {
                  ...
                  if (newItems.length > 0) {
                    showWaitListMessage(
                      `➕ ${newItems.length}개의 대기중 문의가 추가됨`
                    );
                  }
                  return [...prevList, ...newItems];
                }
            ...
            setPersonalMessageList((prevList) => {
              switch (status) {
                ...
                case "ADD": {
                  ...
                  if (newItems.length > 0) {
                    showMatchedListMessage(
                      `➕ ${newItems.length}개의 문의가 추가되었습니다.`
                    );
                  }
                  return [...prevList, ...newItems];
                }
                case "REMOVE": {
                  const removeIds = matchedQnA.map((item) => item.qnaIdx);
                  showMatchedListMessage(
                    `🗑️ 문의 ${removeIds.join(", ")}번이 삭제되었습니다.`
                  );
                  return prevList.filter(
                    (item) => !removeIds.includes(item.qnaIdx)
                  );
                ...
                case "UPDATE": {
                  const updateIds = matchedQnA.map((item) => item.qnaIdx);
                  showMatchedListMessage(
                    `✏️ 문의 ${updateIds.join(", ")}번이 수정되었습니다.`
                  );
                  return prevList.map((item) => {
          ...
          case "DEL_COMM":
            ...
            setQnaComments((prev) => {
              ...
              const filtered = existing.filter(
                (c) => c.qnaReplyIdx !== deleteQnaReplyIdx
              );
              return {
                ...prev,
                [qnaIdx]: filtered,
    ...
    <div style={{ padding: "20px" }}>
      ...
      <div
        style={{
          marginTop: "10px",
          fontSize: "1.2em",
          color: connectionStatus === "연결됨" ? "green" : "red",
        }}
      >
        {connectionStatus}
      </div>
      ...
      <div style={{ marginTop: "20px" }}>
              ...
              {adminList.map((admin, index) => (
                ...
                >
                  ...
                  >
                    ...
                    <div style={{ color: "#555" }}>
                      <strong>매칭 수:</strong> {admin.matched}
                    </div>
                    <div
                      style={{
                        backgroundColor:
                          admin.status === 0 ? "#b0b0b0" : "#4CAF50", // 비활성 상태는 회색, 활성 상태는 녹색
                        color: "white",
                        padding: "4px 12px",
                        borderRadius: "5px",
                        fontWeight: "bold",
        ...
        <button onClick={handleSwitchStatus} style={{ marginLeft: "10px" }}>
          <span role="img" aria-label="switch">
            🔄
          </span>{" "}
          상태 전환
        </button>
        <button onClick={handleTokenReissue}>
        ...
        {personalMessageList.length > 0 && (
          <div style={{ marginTop: "20px" }}>
            <h3>
              <span role="img" aria-label="privacy-key">
                🔐
              </span>{" "}
              개인 메시지 응답 목록:
            </h3>
            <ul style={{ listStyle: "none", padding: 0 }}>
              {personalMessageList.map((msg, idx) => {
                ...
                return (
                  <li
                    key={idx}
                    onClick={() => toggleQnaContent(qna.qnaIdx)}
                    style={{
                      position: "relative", // 뱃지 위치를 위한 상대 위치 설정
                      marginBottom: "10px",
                      padding: "8px",
                  ...
                  >
                    ...
                    >
                      <div>
                        <div>
                          <strong>QnA 번호:</strong> {qna.qnaIdx}
                        </div>
                        <div>
                          <strong>유저 이름:</strong> {qna.userName}
                        </div>
                        <div>
                      ...
                      >
                        ...
                        <div>
                          <strong>상태:</strong> {qna.qnaStatus}
                        </div>
                      </div>
                    </div>
                    ...
                    {isExpanded && (
                      ...
                      >
                        <strong>본문:</strong>
                        <div style={{ marginTop: "5px" }}>
                          {/* React-Quill 뷰어 모드로 QnA 본문 표시 */}
                          <div className="qna-content-viewer">
                            <ReactQuill
                              value={
                                qnaContents[qna.qnaIdx]?.body || "로딩 중..."
                              }
                              readOnly={true}
                              theme="snow"
                              ...
                              return (
                                ...
                                >
                                  <div style={commentBoxStyle}>
                                    ...
                                    >
                                      {/* React-Quill 뷰어 모드로 답변 내용 표시 */}
                                      <div className="comment-content-viewer">
                                        <ReactQuill
