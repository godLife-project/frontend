...
export default function RoutineDetailPage() {
  ...
  useEffect(() => {
    const fetchRoutineData = async () => {
      ...
      try {
        ...
        // 중요: 여기서도 API 응답의 isActive 값을 올바르게 해석하여 설정
        const routineDataFromServer = response.data.message;

        // boolean으로 확실하게 변환 (서버에서 1, '1' 등으로 올 수 있음)
        if (typeof routineDataFromServer.isActive !== "boolean") {
          routineDataFromServer.isActive = Boolean(
            routineDataFromServer.isActive === true ||
              routineDataFromServer.isActive === 1 ||
              routineDataFromServer.isActive === "1"
          );
        }

        // 상태 업데이트
        setRoutineData(routineDataFromServer);
        console.log("초기 로딩된 루틴 데이터:", routineDataFromServer);
  ...
  const handleRoutineAction = async (action) => {
    try {
      ...
      // 먼저 UI 상태를 즉시 업데이트 (사용자 경험 개선)
      setRoutineData((prevData) => ({
        ...prevData,
        isActive: action === "start",
      }));

      // API 호출 함수 - 토큰 재발급 로직을 포함
      const callRoutineAPI = async (currentToken) => {
        try {
      ...
      // API 호출 (토큰 재발급 로직 포함)
      const response = await callRoutineAPI(token);
      ...
      if (response.data.success) {
        ...
        // API 응답의 isActive 값을 올바르게 해석하여 설정
        const updatedData = updatedResponse.data.message;
        ...
        console.log("업데이트된 루틴 데이터:", updatedData);
